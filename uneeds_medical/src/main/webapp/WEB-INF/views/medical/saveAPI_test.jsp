			...
			$.get("hospitalSidoCode", function(data) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					ul.append("<li><input type='text' value='"
							+ item.find("addrCd").text()
							+ "' name='sidocode' id='sicode" + i
							+ "'/><input type='text' value='"
							+ item.find("addrCdNm").text()
							+ "' name='siname' id='siname" + i + "'/></li>");
				...
				$.ajax({
					...
					success : function(data, state) {
						...
						for (var i = 0; i < it.length; i++) {
							ite = $(it[i]);
							gd.append("<li>" + i + "<input type='text' value='"
									+ ite.find("addrCd").text()
									+ "' name='sidocode' id='guncode" + i
									+ "'/><input type='text' value='"
									+ ite.find("addrCdNm").text()
									+ "' name='siname' id='gunname" + i
									+ "'/></li>");
							console.log(parseInt($("#guncode" + i).val()));

							$.post("insertGun", {
								guncode : parseInt($("#guncode" + i).val()),
								gunname : $("#gunname" + i).val()
							}, function(date) {
							});
						}
					},
		...
		function kindsList() {
			$.ajax({
				...
				success : function(data, state) {
					...
					for (var i = 0; i < it.length; i++) {
						ite = $(it[i]);
						gd.append("<li><input type='text' value='"
								+ ite.find("dgsbjtCd").text()
								+ "' name='kindscode' id='kindcode" + i
								+ "'/><input type='text' value='"
								+ ite.find("dgsbjtCdCmmt").text()
								+ "' name='kindname' id='kindname" + i
								+ "'/></li>");
						$.post("insertKinds", {
							mknumber : $("#kindcode" + i).val(),
							mkname : $("#kindname" + i).val()
						}, function(date) {
						});
					}
				},
			...
			$.ajax({
				...
				success : function(data, state) {
					...
					for (var i = 0; i < it.length; i++) {
						...
						gd.append("<li><input type='text' value='"
								+ ite.find("clCd").text()
								+ "' name='themacode' id='themacode" + i
								+ "'/><input type='text' value='"
								+ ite.find("clCdNm").text()
								+ "' name='themaname' id='themaname" + i
								+ "'/></li>");

						$.post("insertThema", {
							mtnumber : $("#themacode" + i).val(),
							mtname : $("#themaname" + i).val()
						}, function(date) {
						});
